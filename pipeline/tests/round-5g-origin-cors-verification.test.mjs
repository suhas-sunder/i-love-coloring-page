...
const REQUIRED_JSON = [
  "pipeline/manifests/round-5g-project-context-check.json",
  "pipeline/manifests/round-5g-public-asset-base-validation.json",
  "pipeline/manifests/round-5g-cors-failure-audit.json",
  "pipeline/manifests/round-5g-origin-aware-url-results.json",
  "pipeline/manifests/round-5g-svg-webp-public-url-results.json",
  "pipeline/manifests/round-5g-browser-public-cors-qa-results.json",
  "pipeline/manifests/round-5g-download-format-readiness.json",
  "pipeline/manifests/round-5g-final-upload-readiness.json",
  "pipeline/manifests/round-5g-r2-cors-guidance-update.json",
];

test("Round 5G JSON manifests parse and confirm the expected project context", async () => {
  ...
  const context = await readJson("pipeline/manifests/round-5g-project-context-check.json");
  assert.equal(context.summary.correctRepository, true);
  assert.equal(context.summary.branch, "version-4");
  assert.equal(context.summary.round5fCommitExists, true);
...
test("public asset base validation exists and blocks non-public or missing values", async () => {
  const validation = await readJson("pipeline/manifests/round-5g-public-asset-base-validation.json");

  assert.equal(typeof validation.summary.publicVerificationReady, "boolean");
  assert.equal(validation.summary.publicBaseUrlConfigured, true);
  assert.equal(validation.summary.baseType, "r2.dev");
  assert.equal(validation.summary.publicVerificationReady, true);
  assert.equal(validation.summary.includesColoringPagesPrefix || validation.summary.publicVerificationReady === false, true);
  assert.equal(validation.summary.isR2Dev, true);
  assert.equal(validation.summary.isCustomDomain, false);
...
test("origin-aware verifier sends Origin headers and records both local preview origins", async () => {
  const verifier = await readText("pipeline/scripts/round-5g-verify-svg-webp-public-cors.mjs");
  const audit = await readJson("pipeline/manifests/round-5g-cors-failure-audit.json");
  const originAware = await readJson("pipeline/manifests/round-5g-origin-aware-url-results.json");

  assert.match(verifier, /--origin/);
  assert.match(verifier, /headers:\s*\{\s*Origin:\s*origin\s*\}/);
  assert.equal(audit.summary.round5fVerifierSentOriginHeaders, true);
  assert.ok(originAware.summary.originsChecked.includes("http://localhost:3005"));
  assert.ok(originAware.summary.originsChecked.includes("http://127.0.0.1:3005"));
  assert.equal(originAware.summary.originHeaderSentCount, originAware.summary.checkedOriginRequestCount);
});

test("public URL and Origin-aware SVG CORS results remain honest when verification cannot run", async () => {
  const baseValidation = await readJson("pipeline/manifests/round-5g-public-asset-base-validation.json");
  const urlResults = await readJson("pipeline/manifests/round-5g-svg-webp-public-url-results.json");
  const originAware = await readJson("pipeline/manifests/round-5g-origin-aware-url-results.json");
  ...
  assert.equal(urlResults.summary.checkedUrlCount, 60);
  assert.equal(urlResults.summary.publicUrlResolutionPassed, true);
  assert.equal(urlResults.summary.contentTypesPassed, true);
  assert.equal(urlResults.summary.svgContentTypePassCount, 30);
  assert.equal(urlResults.summary.webpContentTypePassCount, 30);
  assert.ok(["completed", "not_run"].includes(originAware.summary.status));
  ...
  if (!baseValidation.summary.publicVerificationReady) {
    ...
    assert.equal(originAware.summary.status, "not_run");
    assert.equal(originAware.summary.svgCorsPassed, false);
  }
});

test("CORS success is required before conversion and JPG/JPEG/WebP readiness", async () => {
  const originAware = await readJson("pipeline/manifests/round-5g-origin-aware-url-results.json");
  const browserQa = await readJson("pipeline/manifests/round-5g-browser-public-cors-qa-results.json");
  const readiness = await readJson("pipeline/manifests/round-5g-download-format-readiness.json");

  if (originAware.summary.svgCorsPassed && browserQa.summary.browserCanvasExportPassed) {
...
test("final upload readiness and R2 guidance stay SVG plus WebP only", async () => {
  const finalReadiness = await readJson("pipeline/manifests/round-5g-final-upload-readiness.json");
  const guidance = await readJson("pipeline/manifests/round-5g-r2-cors-guidance-update.json");
