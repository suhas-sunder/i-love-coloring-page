...
const REQUIRED_JSON = [
  "pipeline/manifests/round-5d-project-context-check.json",
  "pipeline/manifests/round-5d-public-asset-base-validation.json",
  "pipeline/manifests/round-5d-svg-webp-public-url-results.json",
  "pipeline/manifests/round-5d-svg-cors-results.json",
  "pipeline/manifests/round-5d-browser-svg-webp-public-qa-results.json",
  "pipeline/manifests/round-5d-download-format-readiness.json",
  "pipeline/manifests/round-5d-final-upload-readiness.json",
  "pipeline/manifests/round-5d-r2-cors-content-type-update.json",
];

test("Round 5D JSON manifests parse and confirm the expected project context", async () => {
  ...
  const context = await readJson("pipeline/manifests/round-5d-project-context-check.json");
  assert.equal(context.summary.correctRepository, true);
  assert.equal(context.summary.branch, "version-4");
  assert.equal(context.summary.round5cCommitExists, true);
...
test("public asset base validation exists and blocks non-public or missing values", async () => {
  const validation = await readJson("pipeline/manifests/round-5d-public-asset-base-validation.json");
...
test("public URL and SVG CORS results remain honest when verification cannot run", async () => {
  const baseValidation = await readJson("pipeline/manifests/round-5d-public-asset-base-validation.json");
  const urlResults = await readJson("pipeline/manifests/round-5d-svg-webp-public-url-results.json");
  const corsResults = await readJson("pipeline/manifests/round-5d-svg-cors-results.json");
...
test("CORS success is required before conversion and JPG/JPEG/WebP readiness", async () => {
  const corsResults = await readJson("pipeline/manifests/round-5d-svg-cors-results.json");
  const browserQa = await readJson("pipeline/manifests/round-5d-browser-svg-webp-public-qa-results.json");
  const readiness = await readJson("pipeline/manifests/round-5d-download-format-readiness.json");
...
test("final upload readiness and R2 guidance stay SVG plus WebP only", async () => {
  const finalReadiness = await readJson("pipeline/manifests/round-5d-final-upload-readiness.json");
  const guidance = await readJson("pipeline/manifests/round-5d-r2-cors-content-type-update.json");
