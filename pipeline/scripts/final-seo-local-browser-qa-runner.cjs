...
const RUN_ID = "final-seo-local-browser-qa";
const SCREENSHOT_DIR = "pipeline/review/final-seo-local-acceptance/screenshots";
const ROUTES = [
  "/",
  "/coloring-pages",
...
const VIEWPORTS = [
  ...
  { label: "1536", width: 1536, height: 1000 },
  { label: "1600", width: 1600, height: 1000 },
  { label: "1739", width: 1739, height: 1000 },
  { label: "1740", width: 1740, height: 1000 },
...
const SCREENSHOT_VIEWPORTS = new Set(["390", "1440", "1536", "1920", "2560"]);
...
async function main() {
  ...
  try {
    ...
    for (const viewport of [
      ...
      const railContext = await browser.newContext({ viewport: { width: viewport.width, height: viewport.height } });
      const railBaseUrl = await installStaticExportRoutes(railContext, build.outDir);
      try {
        adRailViewportResults.push(await runAdRailCheck(railContext, railBaseUrl, viewport.label));
      } finally {
  ...
  const summary = buildSummary(routeResults, modalResult, moreMenuResult, adRailViewportResults, consoleIssues, source);
  const payload = {
  ...
  await writeJson("pipeline/manifests/final-seo-local-browser-qa-results.json", payload);
  await writeText("pipeline/reports/final-seo-local-browser-qa-report.md", renderReport(payload));
...
async function runAdRailCheck(context, baseUrl, viewportLabel) {
  const page = await context.newPage();
  try {
    await page.goto(`${baseUrl}/`, { waitUntil: "domcontentloaded", timeout: 45000 });
    await page.waitForLoadState("load", { timeout: 45000 }).catch(() => {});
...
function buildSummary(routeResults, modalResult, moreMenuResult, adRailViewportResults, consoleIssues, source) {
  ...
  const railsByViewport = new Map(adRailViewportResults.map((result) => [result.viewport, result]));
  const compactRailViewports = ["1536", "1600", "1739"];
  const compactSideRailsVisible = compactRailViewports.every((label) => {
    const result = railsByViewport.get(label);
    return result?.leftRailVisible && result?.rightRailVisible && !result?.horizontalOverflow;
  });
  const fullDesktopRailsVisible = Boolean(railsByViewport.get("1740")?.leftRailVisible && railsByViewport.get("1740")?.rightRailVisible);
  const smallerDesktopRailsHidden = Boolean((railsByViewport.get("1440")?.visibleRails || 0) === 0);
  const summary = {
    ...
    compactSideRailsVisible,
    fullDesktopRailsVisible,
    smallerDesktopRailsHidden,
    noHorizontalOverflow: routeResults.every((route) => !route.horizontalOverflow) && adRailViewportResults.every((result) => !result.horizontalOverflow),
...
function renderReport(payload) {
  return `# Final SEO Local Browser QA

${renderTable(Object.entries(payload.summary).map(([key, value]) => [key, value]))}

## Side Rail Viewports
${renderTable(payload.adRailViewportResults.map((result) => [result.viewport, `rails=${result.visibleRails}; slots=${result.visibleAdSlots}; overflow=${result.horizontalOverflow}`]))}
