...
const RUN_ID = "round-5d-browser-svg-webp-public-qa";
const SCREENSHOT_DIR = path.join(REPO_ROOT, "pipeline", "review", "round-5d", "screenshots");
const QA_MANIFEST = path.join(REPO_ROOT, "pipeline", "manifests", "round-5d-browser-svg-webp-public-qa-results.json");
const QA_REPORT = path.join(REPO_ROOT, "pipeline", "reports", "round-5d-browser-svg-webp-public-qa-report.md");
const DOWNLOAD_READINESS = path.join(REPO_ROOT, "pipeline", "manifests", "round-5d-download-format-readiness.json");
const DOWNLOAD_REPORT = path.join(REPO_ROOT, "pipeline", "reports", "round-5d-download-format-readiness.md");
...
async function main() {
  ...
  const appUrl = normalizeUrl(args.appUrl || process.env.ROUND_5D_APP_URL || "http://127.0.0.1:3005");
  const publicBaseUrl = normalizeUrl(args.publicBaseUrl || process.env.NEXT_PUBLIC_COLORING_ASSET_BASE_URL || "");
  const baseValidation = await readJsonIfExists("pipeline/manifests/round-5d-public-asset-base-validation.json");
  const urlResults = await readJsonIfExists("pipeline/manifests/round-5d-svg-webp-public-url-results.json");
  const corsResults = await readJsonIfExists("pipeline/manifests/round-5d-svg-cors-results.json");
...
async function updateDownloadReadiness(browserPayload) {
  ...
  await writeText(DOWNLOAD_REPORT, `# Round 5D Download Format Readiness
...
function renderReport(payload) {
  return `# Round 5D Browser SVG + WebP Public QA Report
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else throw new Error(`Unknown Round 5D browser QA option: ${arg}`);
  }
  return parsed;
}
