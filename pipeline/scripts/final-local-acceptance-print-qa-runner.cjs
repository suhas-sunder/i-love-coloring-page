...
const RUN_ID = "final-local-acceptance-print-qa";
const ARTIFACT_DIR = "pipeline/review/final-local-acceptance/print";
const SAMPLE_SPECS = [
  {
...
async function main() {
  ...
  await writeJson("pipeline/manifests/final-local-acceptance-print-qa-results.json", payload);
  await writeText("pipeline/reports/final-local-acceptance-print-qa-report.md", renderReport(payload));
