...
const RUN_ID = "round-5g-browser-public-cors-qa";
const SCREENSHOT_DIR = path.join(REPO_ROOT, "pipeline", "review", "round-5g", "screenshots");
const QA_MANIFEST = path.join(REPO_ROOT, "pipeline", "manifests", "round-5g-browser-public-cors-qa-results.json");
const QA_REPORT = path.join(REPO_ROOT, "pipeline", "reports", "round-5g-browser-public-cors-qa-report.md");
const DOWNLOAD_READINESS = path.join(REPO_ROOT, "pipeline", "manifests", "round-5g-download-format-readiness.json");
const DOWNLOAD_REPORT = path.join(REPO_ROOT, "pipeline", "reports", "round-5g-download-format-readiness.md");
...
async function main() {
  ...
  const appUrl = normalizeUrl(args.appUrl || process.env.ROUND_5G_APP_URL || "http://127.0.0.1:3005");
  const publicBaseUrl = normalizeUrl(args.publicBaseUrl || process.env.NEXT_PUBLIC_COLORING_ASSET_BASE_URL || "");
  const baseValidation = await readJsonIfExists("pipeline/manifests/round-5g-public-asset-base-validation.json");
  const urlResults = await readJsonIfExists("pipeline/manifests/round-5g-origin-aware-url-results.json");
  ...
  try {
    ...
    conversionResult = await runCanvasConversionProbe(browser, appUrl, publicBaseUrl);
  } finally {
    await browser.close();
  }
  ...
  const summary = {
    ...
    pngBlobExportPassed: conversionResult.exports?.png?.passed === true,
    jpegBlobExportPassed: conversionResult.exports?.jpeg?.passed === true,
    webpBlobExportPassed: conversionResult.exports?.webp?.passed === true,
...
async function collectPageMetrics(page) {
  return page.evaluate(() => {
    ...
    const visibleBrokenImages = failedImages.filter((img) => {
    ...
    return {
      ...
      brokenImageCount: visibleBrokenImages.length,
...
async function runCanvasConversionProbe(browser, appUrl, publicBaseUrl) {
    ...
    const result = await page.evaluate(async (url) => {
      ...
      try {
        ...
        return {
          ...
          exports,
          passed: exports.png.passed,
          details: exports.png.passed ? `PNG blob size ${exports.png.byteLength}` : "PNG toBlob returned null or unusable blob.",
        };
      } catch (error) {
...
async function writeNotRun({ appUrl, publicBaseUrl, reason, baseValidation, urlResults }) {
  const payload = {
    generatedAt: new Date().toISOString(),
    runId: RUN_ID,
    appUrl,
    ...
    summary: {
      ...
      pngBlobExportPassed: false,
      jpegBlobExportPassed: false,
      webpBlobExportPassed: false,
    ...
    pages: [],
    screenshotPaths: [],
    blockers: [reason, ...(baseValidation?.blockers || []), ...(urlResults?.blockers || [])],
  };
  await writeJson(QA_MANIFEST, payload);
  await writeText(QA_REPORT, renderReport(payload));
  await updateDownloadReadiness(payload);
...
async function updateDownloadReadiness(browserPayload) {
  ...
  existing.summary.jpgJpegWebpControlsRemainHidden = true;
  ...
  await writeText(DOWNLOAD_REPORT, `# Round 5G Download Format Readiness
...
function renderReport(payload) {
  return `# Round 5G Browser SVG + WebP Public QA Report

- Status: ${payload.summary.publicBrowserQaStatus}
- Public WebP renders: ${payload.summary.publicWebpRenders}
- Non-uploaded items fall back without visible broken images: ${payload.summary.nonUploadedItemsFallbackGracefully}
- No visible broken image icons: ${payload.summary.noBrokenImageIcons}
...
- PNG blob export passed: ${payload.summary.pngBlobExportPassed}
- JPEG blob export passed: ${payload.summary.jpegBlobExportPassed}
- WebP blob export passed: ${payload.summary.webpBlobExportPassed}
...
${payload.pages.map((page) => `- ${page.route}: loaded WebP images ${page.webpImageCount}, hidden failed image elements ${page.failedImageElementCount ?? 0}, visible broken images ${page.brokenImageCount}, horizontal overflow ${page.horizontalOverflow}`).join("\n") || "- None"}
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else throw new Error(`Unknown Round 5G browser QA option: ${arg}`);
  }
  return parsed;
}
