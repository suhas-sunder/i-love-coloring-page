...
const RUN_ID = "final-local-rerun-print-qa";
const ARTIFACT_DIR = "pipeline/review/final-local-rerun/print";
const SAMPLE_SPECS = [
  {
...
async function main() {
  ...
  await writeJson("pipeline/manifests/final-local-rerun-print-qa-results.json", payload);
  await writeText("pipeline/reports/final-local-rerun-print-qa-report.md", renderReport(payload));
