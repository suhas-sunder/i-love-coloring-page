...
const RUN_ID = "final-local-rerun-browser-qa";
const SCREENSHOT_DIR = "pipeline/review/final-local-rerun/screenshots";
const ROUTES = [
  "/",
  "/coloring-pages",
...
async function main() {
  ...
  await writeJson("pipeline/manifests/final-local-rerun-static-export-results.json", staticExport);
  await writeText("pipeline/reports/final-local-rerun-static-export-report.md", renderStaticExportReport(staticExport));
  ...
  await writeJson("pipeline/manifests/final-local-rerun-link-section-acceptance.json", linkSectionAcceptance);
  await writeText("pipeline/reports/final-local-rerun-link-section-acceptance.md", renderLinkSectionReport(linkSectionAcceptance));

  const adPlaceholderQa = buildAdPlaceholderQa(source, routeResults);
  await writeJson("pipeline/manifests/final-local-rerun-ad-placeholder-qa.json", adPlaceholderQa);
  await writeText("pipeline/reports/final-local-rerun-ad-placeholder-qa.md", renderAdReport(adPlaceholderQa));
  ...
  await writeJson("pipeline/manifests/final-local-rerun-browser-qa-results.json", payload);
  await writeText("pipeline/reports/final-local-rerun-browser-qa-report.md", renderBrowserReport(payload));
...
async function inspectStaticExport(build, source) {
  ...
  const summary = {
    ...
    htmlSitemapPageGenerated: outputContainsRoute(files, "/sitemap"),
    sitemapXmlExists: files.some((file) => file.endsWith(`${path.sep}sitemap.xml`)),
    imageSitemapXmlExists: files.some((file) => file.endsWith(`${path.sep}image-sitemap.xml`)),
    robotsTxtExists: files.some((file) => file.endsWith(`${path.sep}robots.txt`)),
  ...
  return {
    generatedAt: new Date().toISOString(),
    runId: "final-local-rerun-static-export-results",
    build,
...
async function runDesktopInteractionChecks(context, baseUrl, source) {
  ...
  try {
    ...
    results.htmlSitemap = await checkHtmlSitemap(page, baseUrl, source.runtimeHubs.hubs.map((hub) => hub.route));
    results.footerSitemap = await checkFooterSitemapLink(page, baseUrl);
...
async function checkLinkSections(page, baseUrl) {
  ...
  return page.evaluate(() => {
    ...
    const linkInfo = (selector, labelSelector, countSelector) => {
      ...
      return { count: links.length, collisions, countAlignRight, ellipsized, pillLike };
    };
    ...
    const seoRelated = linkInfo(".seo-related-link", ".seo-related-link-label", ".seo-related-link-count");
    ...
    const seoRelatedListStyle = seoRelatedList ? getComputedStyle(seoRelatedList) : null;
    ...
    return {
      ...
      seoRelated,
      related,
      seoRelatedListDisplay: seoRelatedListStyle?.display || "",
      seoRelatedListColumnGap: seoRelatedListStyle?.columnGap || "",
      seoRelatedListRowGap: seoRelatedListStyle?.rowGap || "",
      ...
      passed:
        ...
        seoRelated.countAlignRight &&
        related.countAlignRight &&
        heroRelated.pillLike === 0 &&
        seoRelated.pillLike === 0 &&
        related.pillLike === 0 &&
        !popular.ellipsized &&
        !heroRelated.ellipsized &&
        !seoRelated.ellipsized &&
        !related.ellipsized &&
        seoRelatedListStyle?.display === "grid" &&
...
async function checkHtmlSitemap(page, baseUrl, runtimeHubRoutes) {
  ...
  const metrics = await page.evaluate((expectedRoutes) => {
    ...
    const ellipsizedLabels = links.filter((link) => {
      const label = link.querySelector(".html-sitemap-link-label");
      return label && getComputedStyle(label).textOverflow === "ellipsis";
    }).length;
    return {
...
function buildLinkSectionAcceptance(source, routeResults, interactions) {
  ...
  const summary = {
    ...
    relatedCollectionsPassed:
      ...
      interactions?.popularRelated?.seoRelated?.collisions === 0 &&
      interactions?.popularRelated?.related?.countAlignRight === true &&
      interactions?.popularRelated?.heroRelated?.countAlignRight === true &&
      interactions?.popularRelated?.seoRelated?.countAlignRight === true &&
      interactions?.popularRelated?.related?.pillLike === 0 &&
      interactions?.popularRelated?.heroRelated?.pillLike === 0 &&
      interactions?.popularRelated?.seoRelated?.pillLike === 0,
    ...
    noCrampedPills:
      desktopRoutes.every((result) => result.relatedMinimumGap == null || result.relatedMinimumGap >= 16) &&
      interactions?.popularRelated?.related?.pillLike === 0 &&
      interactions?.popularRelated?.heroRelated?.pillLike === 0 &&
      interactions?.popularRelated?.seoRelated?.pillLike === 0,
  ...
  return {
    generatedAt: new Date().toISOString(),
    runId: "final-local-rerun-link-section-acceptance",
    summary,
    details: {
...
function buildAdPlaceholderQa(source, routeResults) {
  ...
  return {
    generatedAt: new Date().toISOString(),
    runId: "final-local-rerun-ad-placeholder-qa",
    summary,
...
function buildBrowserSummary(routeResults, interactions, consoleErrors, linkSectionAcceptance, adPlaceholderQa) {
  ...
  const summary = {
    ...
    relatedCollectionsNotPillDump: linkSectionAcceptance.summary.noCrampedPills,
    htmlSitemapGroupedReadable: interactions?.htmlSitemap?.passed === true,
    footerSitemapLinkWorks: interactions?.footerSitemap?.passed === true,
...
function renderStaticExportReport(payload) {
  return [
    "# Final Local Rerun Static Export Report",
    "",
    renderTable([
      ...
      ["htmlSitemapPageGenerated", passFail(payload.summary.htmlSitemapPageGenerated)],
      ["sitemapXmlExists", passFail(payload.summary.sitemapXmlExists)],
      ["imageSitemapXmlExists", passFail(payload.summary.imageSitemapXmlExists)],
      ["robotsTxtExists", passFail(payload.summary.robotsTxtExists)],
...
function renderBrowserReport(payload) {
  return [
    ...
    `- HTML sitemap: ${passFail(payload.interactionResults?.htmlSitemap?.passed)}`,
    `- Footer sitemap link: ${passFail(payload.interactionResults?.footerSitemap?.passed)}`,
...
function renderLinkSectionReport(payload) {
  return [
    "# Final Local Rerun Link Section Acceptance",
    "",
...
function renderAdReport(payload) {
  return [
    "# Final Local Rerun Ad Placeholder QA",
    "",
    renderTable(Object.entries(payload.summary).map(([key, value]) => [key, passFail(value)])),
    "",
