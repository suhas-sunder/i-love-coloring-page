...
const RUN_ID = "post-long-tail-r2-browser-qa";
const OUT_DIR = path.join(REPO_ROOT, "out");
const SCREENSHOT_DIR = path.join(REPO_ROOT, "pipeline", "review", "post-long-tail-r2", "screenshots");
const MANIFEST_PATH = path.join(REPO_ROOT, "pipeline", "manifests", "post-long-tail-r2-browser-qa-results.json");
const REPORT_PATH = path.join(REPO_ROOT, "pipeline", "reports", "post-long-tail-r2-browser-qa-report.md");
const ACCEPTANCE_PATH = path.join(REPO_ROOT, "pipeline", "manifests", "post-long-tail-r2-acceptance-gate.json");
const ACCEPTANCE_REPORT_PATH = path.join(REPO_ROOT, "pipeline", "reports", "post-long-tail-r2-acceptance-gate.md");
...
async function main() {
  ...
  const summary = {
    ...
    promotedRoutesChecked: routeResults.filter((result) => promotedSlugs.some((slug) => result.route === `/coloring-pages/${slug}`)).length,
    ...
    adPlaceholderDensityUnchanged: routeResults.every(
      (result) => result.advertisementLabelCount > 0 && result.adInNavigationCount === 0 && result.adInsideGalleryCount === 0,
    ),
...
async function collectRouteMetrics(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      advertisementLabelCount: (bodyText.match(/Advertisement/g) || []).length,
      adInNavigationCount: [...document.querySelectorAll("nav")].filter((nav) => /Advertisement/.test(nav.textContent || "")).length,
      adInsideGalleryCount: [...document.querySelectorAll(".gallery-grid, .gallery-list")].filter((gallery) =>
        /Advertisement/.test(gallery.textContent || ""),
      ).length,
    };
  });
}
...
function renderReport(payload) {
  const rows = Object.entries(payload.summary)
    ...
    .join("\n");
  return `# Post Long-Tail R2 Browser QA Report
